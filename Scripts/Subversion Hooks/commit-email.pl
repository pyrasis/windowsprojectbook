...
# $LastChangedDate: 2007-02-22 09:59:53 +0900 (목, 22 2 2007) $
# $LastChangedBy: dlr $
...
if ($mode eq 'commit')
  {
    ...
    from_to($_, "euc-kr", "utf-8") foreach (@infolines);
...
foreach my $project (@project_settings_list)
  {
    ...
    my $src_encoding    = $project->{src_encoding};
    ...
    if ($diff_wanted and not @difflines)
      {
        ...
        if ($src_encoding)
          {
            from_to($_, $src_encoding, "utf-8") foreach (@difflines);
          }
      }
...
sub usage
{
  ...
  die "usage (commit mode):\n",
      ...
      "options are:\n",
      "  -e encoding           Character set encoding used in sources\n",
