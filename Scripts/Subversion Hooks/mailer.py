...
#
# $HeadURL: http://svn.collab.net/repos/svn/tags/1.4.5/tools/hook-scripts/mailer/mailer.py $
# $LastChangedDate: 2006-09-12 03:00:53 +0900 (Tue, 12 9 2006) $
# $LastChangedBy: dlr $
...
class DiffContent:
  ...
  def __getitem__(self, idx):
    ...
    line = to_utf8(self.pipe.fromchild.readline())
    line = re.sub("\r?\n", "\n", line)
    if not line:
      # wait on the child so we don't end up with a billion zombies
      self.pipe.wait()
